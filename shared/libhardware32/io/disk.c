...
/*void _ata_handler(irs_t* registers) {
...
void read_sectors_ATA_PIO(uint32_t target_address, uint32_t LBA, uint8_t sector_count) {
	...
	port_writeb(0x1F3, (uint8_t) LBA);
	port_writeb(0x1F4, (uint8_t)(LBA >> 8));
	port_writeb(0x1F5, (uint8_t)(LBA >> 16)); 
	...
	for (int j =0;j<sector_count;j++) {
		...
		ATA_wait_DRQ();
		for(int i=0;i<256;i++)
			target[i] = port_readw(0x1F0);
		target+=256;
	}
}

// writes <sector_count> amount of bytes from <bytes> to <LBA>
void write_sectors_ATA_PIO(uint32_t LBA, uint8_t sector_count, uint32_t* bytes) {
	...
	port_writeb(0x1F4, (uint8_t)(LBA >> 8));
	port_writeb(0x1F5, (uint8_t)(LBA >> 16)); 
	port_writeb(0x1F7,0x30); // Send the write command
...
static void ATA_wait_BSY() {
	while(port_readb(0x1F7) & STATUS_BSY);
}
//Wait fot drq to be 1
static void ATA_wait_DRQ() {
	while(!(port_readb(0x1F7)&STATUS_RDY));
}
#endif